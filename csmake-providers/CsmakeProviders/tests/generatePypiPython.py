...
plt = PypiProvider.PullLinksThread(target, 'simple', log)
plt.start()
plt.join()
...
for link, _ in links:
    ...
    plt = PypiProvider.PullLinksThread(target, "simple/%s/"% link, log)
    plt.start()
    plt.join()
...
print "pypiDict = ", repr(pypiDict)
print "outliers = ", repr(outliers)
